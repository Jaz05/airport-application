...
func ProcessPayment(cardNumber int64, securityNumber int, expirationDate string) error {
	...
	// call payment api
	cardValidationFetch := queries.CreateFetch(fmt.Sprintf("api/bank/card_number=%d", cardNumber))
	cardPaymentFetch := queries.CreateFetch(fmt.Sprintf("api/payment/card_number=%d", cardNumber))
